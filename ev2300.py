...
class EV2300:
	...
	def __init__(self):
		self.dev = None
		self.firmwareFilename = "ev2300_default.bin"
		self.buffer = array('B', [0]) * 64

	def __load_firmware(self, dev):
		logger.info("Loading firmware: %s", important(EV2300_FIRMWARE))
		firmware = array('B')
		with open(EV2300_FIRMWARE, 'rb') as f:
	...
	def __check_arguments(self, device_address, register_address):
	...
	def __calculate_crc(self, slice):
		crc = crc8.crc8()
	...
	def smbus_read_word(self, device_address, register_address):
		self.__check_state()
		self.__check_arguments(device_address, register_address)
		...
		self.buffer[9]   = self.__calculate_crc(self.buffer[2:9])
		self.buffer[10]  = 0x55
		self.__fill_with_zeroes(self.buffer[0]) # 11-63 bytes
	...
	def smbus_write_word(self, device_address, register_address, word):
		...
		self.buffer[11]   = self.__calculate_crc(self.buffer[2:9])
		self.buffer[12]   = 0x55
		self.__fill_with_zeroes(self.buffer[0]) # 11-63 bytes
	...
	def smbus_read_block(self, device_address, block_address):
		self.__check_state()
		self.__check_arguments(device_address, block_address)
	...
	def smbus_write_block(self, device_address, block_address, block):
		...
		self.buffer[10+len(block)]    = self.__calculate_crc(self.buffer[2:9])
